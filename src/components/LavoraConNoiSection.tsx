...
const LavoraConNoiSection = () => (
  <section className="relative overflow-hidden" id="lavora-con-noi">
    {/* Dark elegant background inspired by Lavazza */}
    <div className="bg-[hsl(220,37%,15%)] py-20 md:py-28 lg:py-32 px-5 md:px-8">
      {/* Subtle decorative elements */}
      <div className="absolute inset-0 opacity-[0.03]" style={{
        backgroundImage: `radial-gradient(circle at 20% 50%, hsl(25, 40%, 50%) 0%, transparent 50%), radial-gradient(circle at 80% 20%, hsl(25, 40%, 40%) 0%, transparent 40%)`
      }} />

      <div className="relative z-10 max-w-5xl mx-auto">
        {/* Header */}
        <div className="text-center mb-14 md:mb-20">
          <span className="inline-block text-[hsl(25,40%,55%)] text-xs font-bold uppercase tracking-[0.25em] mb-5">
            Opportunità
          </span>
          <h2 className="text-3xl md:text-5xl lg:text-6xl font-display text-white mb-5 leading-tight">
            Lavora con noi
          </h2>
          <div className="w-16 h-[2px] bg-[hsl(25,40%,45%)] mx-auto mb-6" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6 mb-14 md:mb-20">
          {benefits.map((b) => (
            <div
              key={b.title}
              className="group bg-white/[0.04] backdrop-blur-sm border border-white/[0.08] rounded-2xl p-7 md:p-8 text-center space-y-4 transition-all duration-300 hover:bg-white/[0.07] hover:border-white/[0.15]"
            >
              <div className="mx-auto w-14 h-14 rounded-full bg-[hsl(25,40%,45%)]/15 flex items-center justify-center transition-transform duration-300 group-hover:scale-110">
                <b.icon className="w-6 h-6 text-[hsl(25,40%,55%)]" />
              </div>
              <h3 className="font-display text-lg md:text-xl text-white">{b.title}</h3>
        ...
        <div className="text-center mb-16 md:mb-20">
          <Button variant="whatsapp" size="lg" asChild className="gap-2.5 rounded-full text-base h-14 px-10 shadow-lg shadow-[hsl(145,63%,35%)]/20 transition-all duration-300 hover:shadow-xl hover:shadow-[hsl(145,63%,35%)]/30 hover:scale-[1.02]">
            <a
        ...
        <div className="border-t border-white/[0.08] pt-10 md:pt-14">
          <p className="text-white/25 text-[11px] uppercase tracking-[0.3em] text-center mb-8">
          ...
          <div className="flex items-center justify-center gap-10 md:gap-16 lg:gap-20">
            <img
              ...
              className="h-8 md:h-10 lg:h-12 object-contain opacity-50 hover:opacity-80 transition-opacity duration-300"
            />
